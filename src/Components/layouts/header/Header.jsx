...
function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 z-50 w-full h-24 bg-black shadow-[0_8px_32px_0_rgba(234,179,8,0.25)] flex items-center px-2 sm:px-4 md:px-8 font-sans md:font-semibold text-base md:text-lg">
      {/* Logo Left */}
  ...
  <NavLink to="/">
    {({ isActive }) => (
      <img
        ...
        alt="Logo"
        className={`h-14 w-14 md:h-16 md:w-16 rounded-xl border-4 border-yellow-400/70 object-cover shadow-xl transition-all duration-300 hover:scale-105 ${
          isActive ? 'bg-blue-100' : 'bg-blue-200 opacity-60'
        }`}
      />
    )}
  </NavLink>
      ...
      <div className="flex items-center space-x-2 sm:space-x-3">
        <NavLink
          to="/about"
          className={({ isActive }) =>
            `flex px-3 sm:px-4 py-3 rounded-lg font-semibold tracking-wide transition-colors border border-yellow-500/20 text-yellow-400 hover:text-black hover:bg-yellow-400/80 ${isActive ? 'bg-yellow-500/10' : ''}`
          }
        >
          <GrContact />
          {cartItems.length > 0 && (
